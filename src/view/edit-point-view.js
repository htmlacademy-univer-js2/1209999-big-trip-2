...
import { makeFirstLetterBig, createFullDate } from '../utils';
...
const renderEditingTypeTemplate = (currentType) => POINT_TYPES
  .map((type) => `<div class="event__type-item">
    ...
    <label class="event__type-label event__type-label--${type}" for="event-type-${type}-1">${makeFirstLetterBig(type)}</label>
  </div>`)
  .join('');
